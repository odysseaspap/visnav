...
FRAME readFrame( string rgbindex, string depthindex, int index, ParameterReader& pd );
...
int main( int argc, char** argv )
{
    ...
    vector<string> rgb_index;
    vector<string> depth_index;

    ifstream fin("/home/odysseas/Downloads/rgbd_dataset_freiburg1_desk/rgb.txt");

    cout << "begin " << endl;

    string tempax, tempaz;
    fin >> tempax >> tempaz;
    fin >> tempax >> tempaz;
    fin >> tempax >> tempaz;
    fin >> tempax >> tempaz;
    ...
    while (fin.eof() == false) {
        ...
        fin >> tempx >> tempz;
        //cout << tempz << endl;
        rgb_index.push_back(tempz);
    }
    fin.close();

    for(int i=0; i<3; i++)
        cout << rgb_index[i] << endl;

    cout << rgb_index.size() << endl;

    ifstream fin1("/home/odysseas/Downloads/rgbd_dataset_freiburg1_desk/depth.txt");
    ...
    while (fin1.eof() == false) {
        ...
        string temp, temps;
        fin1 >> temp >> temps;
        depth_index.push_back(temps);
    }
    fin1.close();

    for(int i=0; i<3; i++)
        cout << depth_index[i] << endl;
    ...
    int startIndex = 0;
    int endIndex = 750;//rgb_index.size();

    vector< FRAME > keyframes;
    // initialize
    cout<<"Initializing ..."<<endl;
    int currIndex = startIndex;
    FRAME currFrame = readFrame( rgb_index[currIndex], depth_index[currIndex], currIndex, pd );
    ...
    voxel.filter( *tmp );
    pcl::io::savePCDFile( "tum.pcd", *tmp );
...
FRAME readFrame( string rgbindex, string depthindex, int index, ParameterReader& pd )
{
    ...
    ss.clear();
    filename.clear();
    ss<<depthDir<<depthindex<<depthExt;
    ss>>filename;
